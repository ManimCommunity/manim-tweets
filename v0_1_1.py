...
class TwitterScene(Scene):
    def construct(self):
        ...
        self.wait(3)
        cu1=Cutout(box, ball, ball2, ball3, ball4, fill_opacity=0.2, color=GREY, stroke_color=RED)
        self.add(cu1)
        cu1_small = cu1.copy().scale(0.3).to_edge(RIGHT).shift(2*UP)
        self.play(Transform(cu1,cu1_small))

        cu2=Cutout(box, ball, ball2, ball3, ball4, fill_opacity=0.2, color=GREY, stroke_color=RED)
        self.add(cu2)
