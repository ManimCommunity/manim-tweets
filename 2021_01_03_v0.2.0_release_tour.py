...
class ReleaseV020(Scene):
    def construct(self):
        ...
        content_0 = VGroup(
            MarkupText('<tt>CONFIG</tt> dicts have been <b><color col="red">removed!</color></b>').scale(0.7),
            Code("assets/2021_01_03/config_old.py"),
            MarkupText("is now written as").scale(0.5),
            Code("assets/2021_01_03/config_new.py")
        ).arrange(DOWN).to_edge(DOWN, buff=0.25)
        ...
        content_2 = Group(
            VGroup(
                ...
                MarkupText('Discover @ <tt>https://plugins.manim.community</tt>').scale(0.5),
        ...
        ).arrange(DOWN).scale(0.7).to_edge(DOWN, buff=0.25)
        ...
        circle_layout_g = dict([(v, np.append(pos, 0)) for v, pos in nx.layout.circular_layout(g).items()])
        G = Graph(g.nodes, g.edges, layout_scale=2.5)
        content_4 = VGroup(
            MarkupText('<b>New feature:</b> <tt>Graph</tt>').scale(0.7),
            G
        ).arrange(DOWN).to_edge(DOWN, buff=0.25)
        ct = G.get_center()
        ...
        self.play(*[G[v].animate.move_to(ct + 1.5*circle_layout_g[v]) for v in G.vertices])
        self.wait(1.5)
        ...
        content_5 = VGroup(
            ...
            MarkupText("<tt>https://docs.manim.community/en/v0.2.0/changelog.html</tt>").scale(0.3),
        ).arrange(DOWN).to_edge(DOWN, buff=0.25)
